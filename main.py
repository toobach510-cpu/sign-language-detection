...
# Flag to toggle gesture detection
capture_gesture = False

# Initialize the hand detector (replace with your actual HandTrackingModule class)
...
def upload_image():
    ...
    # Process the image (gesture detection or any other processing)
    img = detector.findHands(img)
    posList = detector.findPosition(img, draw=False)

    # Ensure there are at least 21 landmarks before proceeding
    if len(posList) < 21:
        return jsonify({"message": "No hand detected or insufficient landmarks"}), 400

    # Process gesture and send result back
    result = recognize_gesture(posList)

    return jsonify({"message": result})


# Function to recognize gestures (A-Z)
def recognize_gesture(posList):
    result = ""
    fingers = []

    # Define finger positions for gesture recognition
    ...
    # Gesture recognition logic for A to Z
